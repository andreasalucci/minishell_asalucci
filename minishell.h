...
void		check_pipes_2(t_t *t, t_t **token_list, size_t start,
				char *word);
void		add_custom_token(char *value, int type,
				t_t **token_list);
...
void		handle_single_quote(t_t *t, t_t **token_list,
				bool *free_input);
void		handle_double_quote(t_t *t, t_t **token_list,
				bool *free_input);
...
int			add_env(t_env **env, char *key, char *value,
				int exportable);
int			update_env(t_env **env, const char *key,
				const char *value);
...
void		create_heredoc_open(const char *delimiter,
				t_command *cmd, t_env *env, bool *hdc_interrupted);
...
void		init_key_value(t_key_value *data, char *arg,
				char *equal_pos, int is_append);
...
int			handle_input_interruption(bool *hdc_interrupted,
				char *input);
int			handle_eof(char *input);
t_env		*copy_env(char **envp);
void		setup_shell_signals(void);
int			is_cmd_redir_in_2(t_command *cmd, int prev_fd,
...
int			exec_and_wait(t_command *cmds, char *cmd_path,
				char **envp, t_env *env);
char		*ft_strjoin_free(char *s1, char *s2);
char		*ft_strjoin_3(const char *s1, const char *s2,
				const char *s3);
void		cmd_error_no_such_file_or_dir(char *cmd_path,
				char **argv_filtered);
void		cmd_error_is_a_dir(char *cmd_path,
				char **argv_filtered);
